...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      this.rate.isValid = Number.isInteger(this.rate.value) && this.rate.value > 0;
      this.areas.isValid = this.areas.value.length > 0;

      this.isFormValid = this.firstName.isValid && this.firstName.isValid && this.description.isValid && this.rate.isValid && this.areas.isValid;
    },
    submitForm() {
      this.validateForm();
